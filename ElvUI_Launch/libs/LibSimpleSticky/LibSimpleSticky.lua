...
function StickyFrames:SnapFrame(frameA, frameB, left, top, right, bottom)
	...
	if (bA <= tB and bB <= tA) then
		...
		if lA <= (lB + StickyFrames.rangeX) and lA >= (lB - StickyFrames.rangeX) then
			newX = lB + wA
			if frameB == UIParent or frameB == WorldFrame or frameB == ElvUIParent then newX = newX - left/2 + left*2 end
			snap = true
		end
		...
		if rA <= (rB + StickyFrames.rangeX) and rA >= (rB - StickyFrames.rangeX) then
			newX = rB - wA
			if frameB == UIParent or frameB == WorldFrame or frameB == ElvUIParent then newX = newX + right/2 - right*2 end
			snap = true
		end
	...
	if (lA <= rB and lB <= rA) then
		...
		if tA <= (tB + StickyFrames.rangeY) and tA >= (tB - StickyFrames.rangeY) then
			newY = tB - hA
			if frameB == UIParent or frameB == WorldFrame or frameB == ElvUIParent then newY = newY + top/2 - top*2 end
			snap = true
		end
		...
		if bA <= (bB + StickyFrames.rangeY) and bA >= (bB - StickyFrames.rangeY) then
			newY = bB + hA
			if frameB == UIParent or frameB == WorldFrame or frameB == ElvUIParent then newY = newY - bottom/2 + bottom*2 end
			snap = true
		end
