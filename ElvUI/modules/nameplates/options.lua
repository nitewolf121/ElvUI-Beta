...
local function UpdateFilterGroup()
	if not selectedFilter or not C['nameplate']['filter'][selectedFilter] then
	...
	E.Options.args.nameplate.args.filters.args.filterGroup = {
		...
		guiInline = true,
		order = -10,
		get = function(info) return C["nameplate"]['filter'][selectedFilter][ info[#info] ] end,
		set = function(info, value) C["nameplate"]['filter'][selectedFilter][ info[#info] ] = value; NP:UpdateAllPlates(); UpdateFilterGroup() end,		
		args = {
			enable = {
				type = 'toggle',
				order = 1,
			...
			color = {
				...
				name = L['Color'],
				get = function(info)
					local t = C["nameplate"]['filter'][selectedFilter][ info[#info] ]
					if t then
						return t.r, t.g, t.b, t.a
					end
				end,
				set = function(info, r, g, b)
					C["nameplate"]['filter'][selectedFilter][ info[#info] ] = {}
					local t = C["nameplate"]['filter'][selectedFilter][ info[#info] ]
					if t then
						t.r, t.g, t.b = r, g, b
						UpdateFilterGroup()
			...
			customScale = {
				...
				get = function(info) return C["nameplate"]['filter'][selectedFilter][ info[#info] ] end,
				set = function(info, value) C["nameplate"]['filter'][selectedFilter][ info[#info] ] = value; UpdateFilterGroup() end,						
			},
		},	
	}
end
...
E.Options.args.nameplate = {
	...
	desc = L["NamePlates"],
	childGroups = "select",
	get = function(info) return C.nameplate[ info[#info] ] end,
	set = function(info, value) C.nameplate[ info[#info] ] = value; NP:UpdateAllPlates() end,
	args = {
		intro = {
			order = 1,
			type = "description",
		...
		enable = {
			...
			type = "toggle",
			name = L["Enable"],
			set = function(info, value) C.nameplate[ info[#info] ] = value; StaticPopup_Show("CONFIG_RL") end
		},
		general = {
			order = 3,
			type = "group",
			name = L["General"],
			disabled = function() return not NP.OnEnable; end,
			args = {
				width = {
					type = "range",
					order = 1,
				...
				reactions = {
					...
					guiInline = true,
					get = function(info)
						local t = C.nameplate[ info[#info] ]
						return t.r, t.g, t.b, t.a
					end,
					set = function(info, r, g, b)
						C.nameplate[ info[#info] ] = {}
						local t = C.nameplate[ info[#info] ]
						t.r, t.g, t.b = r, g, b
						NP:UpdateAllPlates()
					end,				
		...
		filters = {
			...
			args = {
				...
				selectFilter = {
					...
					values = function()
						filters = {}
						for filter in pairs(C['nameplate']['filter']) do
							filters[filter] = filter
						end
						return filters
					end,
