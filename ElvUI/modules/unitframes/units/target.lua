...
function UF:Update_TargetFrame(frame, db)
	...
	do
		...
		local mini_comboBarY = 0
		if USE_MINI_COMBOBAR then
			mini_comboBarY = -(SPACING+(COMBOBAR_HEIGHT/2))
		end
		
		threat:ClearAllPoints()
		threat:SetBackdropBorderColor(0, 0, 0, 0)
		threat:Point("TOPLEFT", -4, 4+mini_comboBarY)
		threat:Point("TOPRIGHT", 4, 4+mini_comboBarY)
		threat:Point("BOTTOMLEFT", -4, -4)
		threat:Point("BOTTOMRIGHT", 4, -4)

		if USE_POWERBAR_OFFSET then
			threat:Point("TOPRIGHT", 4-POWERBAR_OFFSET, 4+mini_comboBarY)
	...
	do
		...
		if USE_COMBOBAR and not frame:IsElementEnabled('CPoints', unit) then
			frame:EnableElement('CPoints', unit)
		elseif not USE_COMBOBAR and frame:IsElementEnabled('CPoints', unit) then
			frame:DisableElement('CPoints', unit)	
			CPoints:Hide()
		end				
	end
