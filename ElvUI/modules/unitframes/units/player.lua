...
function UF:Update_PlayerFrame(frame, db)
	...
	do
		...
		castbar:Point("TOPRIGHT", frame, "BOTTOMRIGHT", -BORDER, -(BORDER*2+BORDER))
	...
	do
		if E.myclass == "PALADIN" then
			...
			if USE_CLASSBAR and not frame:IsElementEnabled('HolyPower', unit) then
				frame:EnableElement('HolyPower', unit)
				bars:Show()
			elseif not USE_CLASSBAR and frame:IsElementEnabled('HolyPower', unit) then
				frame:DisableElement('HolyPower', unit)	
				bars:Hide()
			end		
		...
		elseif E.myclass == "WARLOCK" then
			...
			if USE_CLASSBAR and not frame:IsElementEnabled('SoulShards', unit) then
				frame:EnableElement('SoulShards', unit)
				bars:Show()
			elseif not USE_CLASSBAR and frame:IsElementEnabled('SoulShards', unit) then
				frame:DisableElement('SoulShards', unit)
				bars:Hide()
			end					
		elseif E.myclass == "DEATHKNIGHT" then
			...
			if USE_CLASSBAR and not frame:IsElementEnabled('Runes', unit) then
				frame:EnableElement('Runes', unit)
				runes:Show()
			elseif not USE_CLASSBAR and frame:IsElementEnabled('Runes', unit) then
				frame:DisableElement('Runes', unit)	
				runes:Hide()
			end					
		elseif E.myclass == "SHAMAN" then
			...
			if USE_CLASSBAR and not frame:IsElementEnabled('TotemBar', unit) then
				frame:EnableElement('TotemBar', unit)
				totems:Show()
			elseif not USE_CLASSBAR and frame:IsElementEnabled('TotemBar', unit) then
				frame:DisableElement('TotemBar', unit)	
				totems:Hide()
			end					
		elseif E.myclass == "DRUID" then
			...
			if USE_CLASSBAR and not frame:IsElementEnabled('EclipseBar', unit) then
				frame:EnableElement('EclipseBar', unit)
			elseif not USE_CLASSBAR and frame:IsElementEnabled('EclipseBar', unit) then
				frame:DisableElement('EclipseBar', unit)	
			end					
		end
	end
	...
	--Combat Fade
	do
		if db.combatfade and not frame:IsElementEnabled('CombatFade', unit) then
			frame:EnableElement('CombatFade', unit)
		elseif not db.combatfade and frame:IsElementEnabled('CombatFade', unit) then
			frame:DisableElement('CombatFade', unit)
		end		
	end
	...
	do
		...
		altpower.Smooth = C['unitframe'].smoothbars
		
		if db.altpower.enable and not frame:IsElementEnabled('AltPowerBar', unit) then
			frame:EnableElement('AltPowerBar', unit)
		elseif not db.altpower.enable and frame:IsElementEnabled('AltPowerBar', unit) then
			frame:DisableElement('AltPowerBar', unit)
		end			
	end
