...
bar:Point('RIGHT', ElvUI_Bar5, 'LEFT', -4, 0)
...
function AB:PositionAndSizeBarPet()
	...
	local size = C['actionbar'].altbuttonsize
	local point = C['actionbar']['barPet'].point
	local numColumns = math.ceil(numButtons / buttonsPerRow);
	local widthMult = C['actionbar']['barPet'].widthMult
	local heightMult = C['actionbar']['barPet'].heightMult
	...
	if C['actionbar']['barPet'].enabled then
		bar:SetScale(1)
		bar:SetAlpha(1)
	else
	...
	if C['actionbar']['barPet'].backdrop == true then
		bar.backdrop:Show()
	else
		bar.backdrop:Hide()
	end
	...
	for i=1, NUM_PET_ACTION_SLOTS do
		...
		if C['actionbar']['barPet'].mouseover == true then
			bar:SetAlpha(0)
	...
	RegisterStateDriver(bar, "show", C['actionbar']['barPet'].visibility)
end
...
function AB:CreateBarPet()
	...
	self:CreateMover(bar, 'Pet Bar', 'barPet')
