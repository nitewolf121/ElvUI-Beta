...
function AB:PositionAndSizeBar4()
	...
	local buttonsPerRow = C['actionbar']['bar4'].buttonsPerRow
	local numButtons = C['actionbar']['bar4'].buttons
	local size = C['actionbar'].buttonsize
	local point = C['actionbar']['bar4'].point
	local numColumns = math.ceil(numButtons / buttonsPerRow);
	local widthMult = C['actionbar']['bar4'].widthMult
	local heightMult = C['actionbar']['bar4'].heightMult
	...
	if C['actionbar']['bar4'].enabled then
		bar:SetScale(1)
		bar:SetAlpha(1)
	else
	...
	if C['actionbar']['bar4'].backdrop == true then
		bar.backdrop:Show()
	else
		bar.backdrop:Hide()
	end
	...
	for i=1, NUM_ACTIONBAR_BUTTONS do
		...
		if C['actionbar']['bar4'].mouseover == true then
			bar:SetAlpha(0)
	...
	RegisterStateDriver(bar, "show", C['actionbar']['bar4'].visibility)
end
...
function AB:CreateBar4()
	...
	self:CreateMover(bar, 'ActionBar 4', 'bar4')
