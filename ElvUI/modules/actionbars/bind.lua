...
function AB:BindUpdate(button, spellMacro)
	...
	if spellmacro=="SPELL" then
		...
		GameTooltip:Show()
		GameTooltip:SetScript("OnHide", function(bind)
			bind:SetOwner(bind, "ANCHOR_NONE")
			bind:SetPoint("BOTTOM", bind, "TOP", 0, 1)
			bind:AddLine(bind.button.name, 1, 1, 1)
			bind.button.bindings = {GetBindingKey(spellmacro.." "..bind.button.name)}
			if #bind.button.bindings == 0 then
				bind:AddLine(L["No bindings set."], .6, .6, .6)
			else
				bind:AddDoubleLine(L["Binding"], L["Key"], .6, .6, .6, .6, .6, .6)
				for i = 1, #bind.button.bindings do
					bind:AddDoubleLine(i, bind.button.bindings[i])
				end
			end
			bind:Show()
			bind:SetScript("OnHide", nil)
		end)
...
function AB:RegisterMacro(addon)
	if addon == "Blizzard_MacroUI" then
		for i=1,36 do
			...
			if not self.hooks[b] then
				self:HookScript(b, "OnEnter", function(b) self:BindUpdate(b, "MACRO") end)
			end
		end	
	end
end
...
function AB:LoadKeyBinder()
	...
	for i=1,12 do
		...
		self:HookScript(b, "OnEnter", function(b) self:BindUpdate(b, "SPELL") end)
	end
