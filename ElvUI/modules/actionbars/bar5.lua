...
function AB:PositionAndSizeBar5()
	...
	local buttonsPerRow = C['actionbar']['bar5'].buttonsPerRow;
	local numButtons = C['actionbar']['bar5'].buttons;
	local size = E:Scale(C['actionbar'].buttonsize);
	local point = C['actionbar']['bar5'].point;
	local numColumns = ceil(numButtons / buttonsPerRow);
	local widthMult = C['actionbar']['bar5'].widthMult;
	local heightMult = C['actionbar']['bar5'].heightMult;
	...
	if C['actionbar']['bar5'].enabled then
		bar:SetScale(1);
		bar:SetAlpha(1);
	else
	...
	if C['actionbar']['bar5'].backdrop == true then
		bar.backdrop:Show();
	else
		bar.backdrop:Hide();
	end
	...
	for i=1, NUM_ACTIONBAR_BUTTONS do
		...
		if C['actionbar']['bar5'].mouseover == true then
			bar:SetAlpha(0);
	...
	RegisterStateDriver(bar, "show", C['actionbar']['bar5'].visibility);
end
...
function AB:CreateBar5()
	...
	self:CreateMover(bar, 'ActionBar 5', 'bar5');
