...
local function GetTemplate(t)
	backdropa = 1
	if t == "ClassColor" or C["core"].classtheme == true  then
...
local function SetTemplate(f, t, texture)
	...
	if not f.backdropTexture and t ~= 'Transparent' then
		...
		backdropTexture:SetDrawLayer("BORDER", -8)
...
local function SetFontTemplate(fs, font, fontSize, fontStyle)
	fs.font = font
	fs.fontSize = fontSize
	fs.fontStyle = fontStyle
	
	if not font then font = E["media"].normFont end
	if not fontSize then fontSize = C["core"].fontsize end
